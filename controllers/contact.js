...
exports.sendFriendRequest = async (req, res) => {
  ...
  try {
    ...
    if (existingRelationship) {
      ...
      return res.status(200).json({ message: 'Friend request already exists', data: existingRelationship });
    }
...
exports.aceptFriendRequest = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Accepted successfully', data: updateAcceptFriendRequest });
  } catch (error) {
    res.status(500).json({ error: 'Internal server error!' });
  }
}
...
exports.getFriendRequestUser = async (req, res) => {
  ...
  try {
    ...
    const friendRequestsWithUser = friendRequests.map(request => {
      const requestUser = requestUsers.find(user => user.id === request.client);
      return {
        ...request,
        friend: requestUser
      };
...
exports.getAllFriendUser = async (req, res) => {
  ...
  try {
    ...
    const friendsWithUserInfo = friendsInfo.map(friend => {
      ...
      const relationship = friendsWithStatus.find(relationship => relationship.friends.includes(friend.id));
...
exports.removeFriend = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Friend removed successfully', data: updatedRelationshipRecord });
  } catch (error) {
    // Xử lý lỗi
...
exports.getFriendRequested = async (req, res) => {
  ...
  try {
    ...
    const friendRequestsWithUser = friendRequests.map(request => {
      const requestUser = requestUsers.find(user => user.id === request.friends.find(id => id !== session.id));
      return {
        ...request,
        friend: requestUser
      };
